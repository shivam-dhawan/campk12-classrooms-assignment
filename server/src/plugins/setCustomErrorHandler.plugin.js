  ...
  fastify.setErrorHandler(function customErrorHandler(error, request, reply) {
    ...
    if (error.isAxiosError) {
      ...
      if (typeof error.response.data.error == 'object')
        ...
        error = axiosError.response.data.errors[0];
      else {
    ...
    switch (errorName) {
      case 'SequelizeUniqueConstraintError':
        let sequelizeError = error.errors[0];
        delete sequelizeError.instance;
        return fastify.badResponse(request, reply, sequelizeError);
